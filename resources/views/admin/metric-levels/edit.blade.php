...
@section('title', 'Edit MetricLevel')

@section('content')
<div class="card">
    <h1>Edit MetricLevel</h1>

    <form method="POST" action="{{ route('lfl.admin.metric-levels.update', $level) }}" style="display: grid; gap: 20px; max-width: 800px;">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select id="gamed_metric_id" name="gamed_metric_id" required style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                ...
                @foreach($metrics as $metric)
                    <option value="{{ $metric->id }}" {{ old('gamed_metric_id', $level->gamed_metric_id) == $metric->id ? 'selected' : '' }}>{{ $metric->name }}</option>
                @endforeach
            </select>
            @error('gamed_metric_id')
        ...
        <div style="display: flex; gap: 10px; margin-top: 20px;">
            <button type="submit" style="background: #007bff; color: white; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">Update MetricLevel</button>
            <a href="{{ route('lfl.admin.metric-levels') }}" style="background: #6c757d; color: white; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; text-decoration: none; display: inline-block;">Cancel</a>
        </div>
    </form>
</div>
@endsection
